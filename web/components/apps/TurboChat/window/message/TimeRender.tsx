...
const TimeRender = ({ messages, message, index }: Props) => {
	...
	const curTime = new Date(message.createAt ?? '').getTime()
	...
	if (curTime - prevTime > 5 * ONE_MINUTE) {
		if (isSameDay) {
			...
			return <p className="text-xs text-center text-gray-400">{curDate.format('YYYY/MM/DD HH:mm').toLocaleString()}</p>
		}
	}
	return null
}
